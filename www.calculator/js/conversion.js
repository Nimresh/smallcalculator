(() => {
  ...
  window.calcul = function calcul() {
    ...
    const rawValue = String(form.fromVal.value ?? "").replace(/,/g, "").trim();
    const value = Number(rawValue);
    if (rawValue === "" || !Number.isFinite(value)) {
      form.toVal.value = "";
      if (output) output.innerHTML = '<span class="calc-error">Enter a valid number to convert.</span>';
      return false;
    }

    const units = window.currentAName || window.lA;
    const fromIndex = Number(form.calFrom.value);
    const toIndex = Number(form.calTo.value);
    if (!Number.isInteger(fromIndex) || !Number.isInteger(toIndex) || fromIndex < 0 || toIndex < 0 || fromIndex >= units.length || toIndex >= units.length) {
      form.toVal.value = "";
      if (output) output.innerHTML = '<span class="calc-error">Choose valid units to convert.</span>';
      return false;
    }
