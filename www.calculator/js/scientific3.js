(() => {
  ...
  function unary(name) {
    if (!expression && !Number.isFinite(lastAnswer)) throw new Error("Enter a valid expression");
    const current = expression || String(lastAnswer);
  ...
  window.r = function r(value) {
    try {
      ...
      if (value === "bk") {
        ...
        render(expression === "" ? 0 : expression);
        return false;
      }
      if (value === "=") {
